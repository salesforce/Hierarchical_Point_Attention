...
class PointnetFPModuleUpSample(nn.Module):
    ''' Modified based on PointnetFPModule, upsample m points to n points with unknow xyz.'''
    def __init__(self, *, mlp: List[int], 
                n: int,
                bn: bool = True):
        super().__init__()
        self.mlp = pt_utils.SharedMLP(mlp, bn=bn)
        self.upsample_n_points = n

    def forward(
            self, xyz_all, known: torch.Tensor, known_feats: torch.Tensor, 
            unknown_xyz=None,
    ) -> torch.Tensor:
        r"""
        Parameters
        ...
        xyz_all: torch.Tensor
            (B, n_total, 3) where N is the total amount of points in a point cloud (e.g., n_total = 40k for scannet)
        known : torch.Tensor
            (B, m, 3) tensor of the xyz positions of the known features
        known_feats : torch.Tensor
            (B, C2, m) tensor of features to be propigated
        unknown_xyz: torch.Tensor
            (B, self.upsample_n_points, 3) tensor of the xyz positions of the (upsampled) unknown features
        Returns
        -------
        new_features : torch.Tensor
            (B, mlp[-1], self.upsample_n_points) tensor of the features of the unknown features
        """
        if unknown_xyz is None:
            xyz_flipped = xyz_all.transpose(1, 2).contiguous()
            inds = pointnet2_utils.furthest_point_sample(xyz_all, self.upsample_n_points)
            new_xyz = pointnet2_utils.gather_operation(
                xyz_flipped, inds
            ).transpose(1, 2).contiguous() # [B, npoint, C]
        else:
            new_xyz = unknown_xyz
            inds = None

        dist, idx = pointnet2_utils.three_nn(new_xyz, known)
        dist_recip = 1.0 / (dist + 1e-8)
        norm = torch.sum(dist_recip, dim=2, keepdim=True)
        weight = dist_recip / norm
        ...
        return new_xyz, new_features.squeeze(-1), inds
