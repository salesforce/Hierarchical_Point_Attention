...
def shift_scale_points(pred_xyz, src_range, dst_range=None):
    ...
    if dst_range is None:
        dst_range = [
            torch.zeros((src_range[0].shape[0], 3), device=src_range[0].device), # min
            torch.ones((src_range[0].shape[0], 3), device=src_range[0].device), # max
        ]
...
def write_oriented_bbox(scene_bbox, out_filename):
    ...
    scene = trimesh.scene.Scene()
    for box in scene_bbox:
        scene.add_geometry(convert_oriented_box_to_trimesh_fmt(box[1]))

    mesh_list = trimesh.util.concatenate(scene.dump())
    mesh_list.metadata["cls"] = [b[0] for b in scene_bbox]
    # save to ply file    
    # mesh_list.export(out_filename)
    with open(out_filename, "wb") as handle:
        pickle.dump({"cls": [b[0] for b in scene_bbox], 
                "mesh": mesh_list}, handle)

    return

def write_pred_gt_pc(pc, pred_bbox, gt_bbox, pred_center, gt_center, out_filename,
                    obj_mask=None, instance_mask=None, 
                    attn_maps=None, downsampled_pc=None, upsampled_pc=None,
                    pc_color=None):
    """Export oriented (around Z axis) scene bbox to meshes
    Args:
        scene_bbox: (N x 7 numpy array): xyz pos of center and 3 lengths (dx,dy,dz)
    ...
    # gt boxes
    gt_scene = trimesh.scene.Scene()
    for box in gt_bbox:
        gt_scene.add_geometry(convert_oriented_box_to_trimesh_fmt(box[1]))
    gt_mesh_list = trimesh.util.concatenate(gt_scene.dump())
    gt_cls = [b[0] for b in gt_bbox]

    # pred boxes
    pred_scene = trimesh.scene.Scene()
    for box in pred_bbox:
        pred_scene.add_geometry(convert_oriented_box_to_trimesh_fmt(box[1]))
    pred_mesh_list = trimesh.util.concatenate(pred_scene.dump())
    pred_cls = [b[0] for b in pred_bbox]

    ## gt center
    # gt_pc = open3d.utility.Vector3dVector(gt_center)
    # gt_center = open3d.geometry.PointCloud()
    # gt_center.points = gt_pc

    ## pred center
    # pred_pc = open3d.utility.Vector3dVector(pred_center)
    # pred_center = open3d.geometry.PointCloud()
    # pred_center.points = pred_pc

    ## pc
    # raw_points = open3d.utility.Vector3dVector(pc)
    # raw_pc = open3d.geometry.PointCloud()
    # raw_pc.points = raw_points
    # raw_pc.colors = open3d.utility.Vector3dVector(pc_color)

    # save to ply file    
    # mesh_list.export(out_filename)

    N = pc.shape[0]
    obj_color = np.ones((N, 3)) * np.array([[0.8, 0.8, 0.8]])
    obj_color += np.expand_dims(obj_mask, axis=1) * np.array([[-0.52, -0.56, -0.26]])

    instance_colors = []
    n_obj = np.max(instance_mask) + 1
    for i in range(n_obj):
        i_color = np.ones((N, 3)) * np.array([[0.8, 0.8, 0.8]])
        i_mask = np.where(instance_mask == i, 1.0, 0.0)
        i_color += np.expand_dims(i_mask, axis=1) * np.array([[-0.52, -0.56, -0.26]])
        instance_colors.append(i_color)
    ...
    for attn in attn_maps:
        # attn: [n_box, 1024]
        np_down = attn.shape[1]
        attn_color = []
        for i in range(attn.shape[0]):
            # min = np.min(attn[i])
            # max = np.max(attn[i])
            # attn_norm = (attn[i] - min) / (max - min)
            # attn_norm = attn_norm - 0.5
            # i_color = np.ones((np_down, 3)) * np.array([[0.55, 0.27, 0.08]])
            # i_color += np.expand_dims(attn_norm, axis=1) * np.array([[0.45, 0.4, 0.3]])
            i_color = np.zeros((np_down, 3))
            prev_thresh = np.max(attn[i])
            obj_attn = attn[i]
            for p, p_color in zip([97, 90, 80, 30, 0], 
                    [[0.9, 0.42, 0.0], [0.72, 0.42, 0.15], [0.63, 0.41, 0.22], [0.54, 0.41, 0.3], [0.36, 0.40, 0.45]]):
                thresh = np.percentile(obj_attn, p)
                # print(thresh)
                p_mask = np.where(obj_attn > thresh, 1.0, 0.0) * \
                                np.where(obj_attn <= prev_thresh, 1.0, 0.0)
                i_color += np.expand_dims(p_mask, axis=1) * np.array([p_color])
                prev_thresh = thresh
            attn_color.append(i_color)
        attn_colors.append(attn_color)

    try:
        gt_box = gt_mesh_list.split()
    except:
        gt_box = gt_mesh_list

    try:
        pred_box = pred_mesh_list.split()
    except:
        pred_box = pred_mesh_list
    ...
    with open(out_filename, "wb") as handle:
        pickle.dump({
                ...
                'obj_mask': obj_color,
                "instance_mask": instance_colors, 
                "attn_maps": attn_colors,
                "downsampled_pc": downsampled_pc,
                "upsampled_pc": upsampled_pc,
            }, handle)
