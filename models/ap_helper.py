...
def parse_predictions(end_points, config_dict, prefix="", size_cls_agnostic=False, return_index=False):
    ...
    for i in range(bsize):
        if config_dict['per_class_proposal']:
        ...
        elif 'return_bbox' in config_dict.keys() and config_dict['return_bbox']:
            batch_pred_map_cls.append([(pred_sem_cls[i, j].item(), pred_center[i, j, :].cpu().numpy(), \
                                        end_points["last_pred_size"][i, j, :].cpu().numpy()) \
                                       for j in range(pred_center.shape[1]) if
                                       pred_mask[i, j] == 1 and obj_prob[i, j] > config_dict['conf_thresh']])
        else:
            batch_pred_map_cls.append([(pred_sem_cls[i, j].item(), pred_corners_3d_upright_camera[i, j], obj_prob[i, j]) \
                                       ...
                                       pred_mask[i, j] == 1 and obj_prob[i, j] > config_dict['conf_thresh']])
        if return_index:
            index_list.append([j for j in range(pred_center.shape[1]) if
                              pred_mask[i, j] == 1 and obj_prob[i, j] > config_dict['conf_thresh']])
    if return_index:
        return batch_pred_map_cls, index_list
...
class APCalculator(object):
    ...
    def __init__(self, area_threshold=None, ap_iou_thresh=0.25, class2type_map=None):
        """
        Args:
    ...
    def compute_metrics(self):
        ...
        """
        rec, prec, ap = eval_det_multiprocessing(self.pred_map_cls, self.gt_map_cls, athresh=self.area_threshold, 
        ...
        if self.area_threshold is not None:
            ...
            for i, k in enumerate(['S', 'M', 'L']):
                for key in sorted(ap.keys()):
                    ...
                    ret_dict[k]['%s Average Precision' % (clsname)] = ap[key][i]
                valid_aps = []
                # for key in ap.keys():
                #     if ap[key][i] > 0:
                #         valid_aps.append(ap[key][i])
                ### unlike the implementation above, which doesn't count absent classes
                ### this one count absent classes' ap as 0
                valid_aps = [ap[key][i] for key in ap.keys()]
                ret_dict[k]['mAP'] = np.mean(valid_aps)
            
                rec_list = []
                for key in sorted(ap.keys()):
                    clsname = self.class2type_map[key] if self.class2type_map else str(key)
                    try:
                        ret_dict[k]['%s Recall' % (clsname)] = rec[key][i][-1]
                        rec_list.append(rec[key][i][-1])
                    except:
                        ret_dict[k]['%s Recall' % (clsname)] = 0
                        rec_list.append(0)
                ret_dict[k]['AR'] = np.mean(rec_list)
        else:
            ret_dict = {}
            for key in sorted(ap.keys()):
                clsname = self.class2type_map[key] if self.class2type_map else str(key)
